...
class Manager(manager.Manager):
    ...
    def _check_domain_trust(self, user_id=None, group_id=None,
                     project_id=None):
        project_domain = self.resource_api.get_project(project_id)
        resource_id = project_domain['domain_id']
        if user_id:
            user_domain = self.identity_api.get_user(user_id)
            actor_id = user_domain['domain_id']
        if group_id:
            group_domain = self.identity_api.get_group(group_id)
            actor_id = group_domain['domain_id']
        if (actor_id != resource_id):
            #if user or group is not within same project domain
            trust_result  = self.driver.get_trust('domain1betadomain2')
                
            trustor_domain_id = trust_result['trustor_domain']
            trustee_domain_id = trust_result['trustee_domain']
            trust_type = trust_result['trust_type']
            # trust_type can be type alpha and gamma
            if (trust_type == 'beta'):
                if (actor_id != trustor_domain_id or resource_id != trustee_domain_id):
                    msg =  _('Specify valid domain trust')
                    raise exception.ValidationError(msg)
        #print "\n -------------------------->>>>>>>>>>>>>>>>>>>>"
        #print self.driver.get_trust('domain1betadomain2')
    ...
    def create_grant(self, role_id, user_id=None, group_id=None,
        ...
        self.role_api.get_role(role_id)
        #print "\n domain_id_list-------->>>>"
        #print self.list_domains_for_user(user_id)
        #if user_id:
        #    print "\n .identity_api.get_user(user_id)----->>>>"
        #    user_domain_id = self.identity_api.get_user(user_id)
        #    print user_domain_id['domain_id']
        if domain_id:
            self.resource_api.get_domain(domain_id)
        if project_id:
            self._check_domain_trust(user_id, group_id, project_id)
            self.resource_api.get_project(project_id)
           # project_domain_id2 = self.resource_api.get_project(project_id)
           # print "\n project_domain_id: ---->>>>>"
           # print project_domain_id2['domain_id']
